...
import { Artikal } from 'src/app/model/artikal.model';
import { Korisnik } from 'src/app/model/korisnik.model';
import { KorisnikService } from 'src/app/services/korisnik.service';

@Component({
  selector: 'app-preduzece-robe-iusluge2',
  templateUrl: './preduzece-robe-iusluge2.component.html',
  styleUrls: ['./preduzece-robe-iusluge2.component.css']
})
export class PreduzeceRobeIUsluge2Component implements OnInit {
  ...
  sledecaStranica(){
    if((this.artikli.length-10*this.stranica)>10)
    this.router.navigate(["preduzece/robe_i_usluge"+(this.sledeca-1)]);

  }
  prethodnaStranica(){
    if(this.stranica-1>0)
    this.router.navigate(["preduzece/robe_i_usluge"+(this.prethodna-1)]);
  ...
  dohvatiArtikle():boolean{
    ...
    if(this.artikli.length>(this.stranica-1)*10) 
    {
    this.artikliZaPrikaz= this.artikli.slice(10*(this.currentPage-1),10*(this.currentPage));
    }
    else{
